...
- In this session, 600 left-eye 640×480 BGR frames: **42.98 FPS**, zero failures, zero dropped frames (second unbuffered run); buffered run: 42.29 FPS, eight dropped. Slow-consumer check: 30 received frames, IDs 0–166, 137 old frames discarded. Full stereo 2560×960: 120 frames at **22.54 FPS**. Direct FFmpeg-to-`dd` with the same left crop/scale took 11.427 s for 600 requested frames, about 52.5 FPS. The older 55.53 FPS result was supplied before this session and was not reproduced by the Python adapter. The performance target remains open.
...
3. Profile the measured camera pipe bottleneck. The full-stereo path currently delivers only 22.54 FPS; keep original calibration dimensions until a validated alternative exists.
