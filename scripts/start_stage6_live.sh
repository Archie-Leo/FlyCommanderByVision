...
set -eo pipefail
[[ "${1:-}" == --confirm-sitl ]] || {
  echo 'Refusing live output. Explicit --confirm-sitl is required.' >&2; exit 2;
}
shift
...
"$ROOT/scripts/check_environment.sh" --live
echo 'Gazebo-only live Intent selected. Safety Pilot must supervise QGC/PX4.' >&2
